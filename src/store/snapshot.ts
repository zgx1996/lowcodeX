...
import { initialState, useState } from "./state";

export const useSnapshot = defineStore('snapshot', {
  state: (): Snapshot => {
    return {
      snapshotList: [cloneDeep(initialState)],
      index: 0,
    }
  },
  actions: {
    snapshot() {
      ...
      if(this.index != this.snapshotList.length - 1) {
        // 说明处于撤销状态中
        this.snapshotList.splice(this.index + 1,0,cloneDeep(state.getState))
        this.index++
      } else {
    ...
    forward(): State {
      if(this.index < this.snapshotList.length) {
        this.index++
      }
      return this.snapshotList[this.index]
    }
